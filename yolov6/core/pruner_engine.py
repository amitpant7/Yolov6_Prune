...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        ratio = float(args.prune_ratio)  # need to make it passable parameter
        steps = 1

        base_macs, base_nparams = tp.utils.count_ops_and_params(model, example_inputs)

        ## Need to ignore entire(detect block) otherwise shape mismatch and not working
        ignored_layers.extend(
            [
                model.backbone.stem,
                model.backbone.ERBlock_2,
                model.detect,
                # model.detect.proj_conv,
                # model.detect.cls_preds,
                # model.detect.reg_preds,
                # model.detect.cls_preds_ab,
                # model.detect.reg_preds_ab,
                # model.detect.reg_convs[-1],
                # model.detect.cls_convs[-1],
            ]
        )
        ...
        print("These layers are being ignored...........\n", ignored_layers)
    ...
    def get_model(self, args, cfg, nc, device):
        if "YOLOv6-lite" in cfg.model.type:
            assert (
                not self.args.fuse_ab
            ), "ERROR in: YOLOv6-lite models not support fuse_ab mode."
            assert (
                not self.args.distill
            ), "ERROR in: YOLOv6-lite models not support distill mode."
