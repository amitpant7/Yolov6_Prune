...
## Yolov6 pruning code on top of the train.py
import argparse
...
from yolov6.core.pruner_engine import Trainer
...
def get_args_parser(add_help=True):
    parser = argparse.ArgumentParser(
        description="YOLOv6 PyTorch Training", add_help=add_help
    )

    parser.add_argument("--prune_ratio", default=0.3, type=float, help="pruning ratio")
    ...
    parser.add_argument(
        "--rect",
        action="store_true",
        help="whether to use rectangular training, default is False",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval-final-only",
        action="store_true",
        help="only evaluate at the final epoch",
    )
    parser.add_argument(
        "--heavy-eval-range",
        default=50,
    ...
    parser.add_argument(
        "--distill_feat", action="store_true", help="distill featmap or not"
    )
    parser.add_argument("--quant", action="store_true", help="quant or not")
    parser.add_argument("--calib", action="store_true", help="run ptq")
    parser.add_argument(
        "--teacher_model_path", type=str, default=None, help="teacher model path"
    )
    parser.add_argument(
        "--temperature", type=int, default=20, help="distill temperature"
    )
    parser.add_argument(
        "--fuse_ab",
        action="store_true",
    ...
    parser.add_argument(
        "--specific-shape", action="store_true", help="rectangular training"
    )
    parser.add_argument(
        "--height", type=int, default=None, help="image height of model input"
    )
    parser.add_argument(
        "--width", type=int, default=None, help="image width of model input"
    )
    parser.add_argument(
        "--cache-ram",
        action="store_true",
...
def check_and_init(args):
    ...
    if args.resume:
        ...
        checkpoint_path = (
            args.resume if isinstance(args.resume, str) else find_latest_checkpoint()
        )
    ...
    if args.specific_shape:
        ...
        args.height = check_img_size(
            args.height, 32, floor=256
